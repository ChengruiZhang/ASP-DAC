...
def test(epoch, test_only=False):
    losses = AverageMeter('Loss')
    top1 = AverageMeter('Acc@1')
    ...
    with torch.no_grad():
        ...
        for batch_idx, (data, target) in enumerate(test_loader):
            ...
            break  # only for power calculation
        ...
        if top1.avg > max_accuracy and test_only == False:
...
if __name__ == '__main__':
    ...
    parser.add_argument('-a', '--architecture', default='VGG16', type=str, help='network architecture',
                        choices=['VGG5', 'VGG9', 'VGG11', 'VGG13', 'VGG16', 'VGG19'])
    ...
    parser.add_argument('--test_only', default = 'False', action='store_true', help='perform only inference')
    ...
    parser.add_argument('--node', default='05', type=str, help='computing node')
    ...
    identifier = 'snn_' + architecture.lower() + '_' + dataset.lower() + '_' + str(timesteps) + '_final-2'
    ...
    if architecture[0:3].lower() == 'vgg': 
        ...
        model = RESNET_SNN_STDB(resnet_name=architecture, activation=activation, labels=labels, timesteps=timesteps,
                                leak=leak, default_threshold=default_threshold, alpha=alpha, beta=beta, dropout=dropout,
                                dataset=dataset)
    ...
    if pretrained_ann:
        ...
        if 'thresholds' in state.keys():
            ...
            thresholds = find_threshold(batch_size=512, timesteps=1000, architecture=architecture)  # Line 48
            model.module.threshold_update(scaling_factor=scaling_factor, thresholds=thresholds[:])
        ...
        for key in state['state_dict'].keys():
            ...
            if key in cur_dict:
                if (state['state_dict'][key].shape == cur_dict[key].shape):
                    ...
                    f.write('\n Size mismatch {}, size of loaded model {}, size of current model {}'.format(key, state[
                        'state_dict'][key].shape, model.state_dict()[key].shape))
            else:
    ...
    for epoch in range(1, epochs):
        ...
        if not args.test_only:
            train(epoch)
        test(epoch, test_only=args.test_only)
    Spike = {}
    for i in model.module.count_spike.keys():
        Spike[i] = model.module.count_spike[i].cpu().numpy()
    name = './spike/VGG5_10_Spike_power_test_32.npy'
    np.save(name, Spike)

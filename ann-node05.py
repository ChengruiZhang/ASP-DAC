...
pydevd_pycharm.settrace('localhost', port=12135, stdoutToServer=True, stderrToServer=True)
...
def train(epoch, loader):
    ...
    aging_lambda2 = 0.001
    aging_lambda3 = 0.005
    # reg_thr     = [-0.10083038, -0.01719275, -0.02412269, -0.00429467, -0.00461976, -0.06036983] # 0.3
    reg_thr = [-0.18058242, -0.0295345, -0.03366761, -0.00672781, -0.00792576, -0.08049597]  # 0.2
    # reg_thr = [-0.06489207, -0.004302091, -0.007418271, -0.0005025283, 0.0003291159, -0.001624121] # standard deviation
    max_weight  = [0.9976442, 0.28460395, 0.20096682, 0.09257056, 0.05684106, 0.19981617]
    ...
    for batch_idx, (data, target) in enumerate(loader):
        ...
        loss_reg = 0
        if reg:
            for model_param in model.modules():
                if isinstance(model_param, (nn.Conv2d, nn.Linear)):
                    # print(model_param.weight.data.shape)
                    # loc = model_param.weight.data > reg_thr[param_count]
                    loc_H = model_param.weight.data >= reg_thr[param_count]
                    loc_L = model_param.weight.data < reg_thr[param_count]
                    loc_M = model_param.weight.data == model_param.weight.data.max()
                    loss_reg += aging_lambda1 * torch.sum(torch.norm(model_param.weight.data[loc_L] - reg_thr[param_count])) \
                                + aging_lambda2 * torch.sum(torch.norm(model_param.weight.data[loc_H] - reg_thr[param_count])) \
                                + aging_lambda3 * torch.sum(torch.norm(model_param.weight.data[loc_M] - max_weight[param_count]))
                    param_count += 1

        loss = F.cross_entropy(output, target) + loss_reg
        #make_dot(loss).view()
        #exit(0)
        loss.backward()
        optimizer.step()
    ...
    with torch.no_grad():
        ...
        if top1.avg>max_accuracy:
            ...
            try:
                ...
                pass
            if reg:
                filename = './trained_models/ann/'+identifier+'-reg-HLB-8-0-node05.pth'
            else:
                filename = './trained_models/ann/' + identifier + '-node05-2.pth'
...
if __name__ == '__main__':
    ...
    parser.add_argument('--devices',                default='2',                type=str,       help='list of gpu device(s)')
